...
async function load() {
  loading.value = true
  const res = await waybillApi.list({ status: 'mechanic_check', per_page: 100 })
...
async function submitCheck() {
  ...
  try {
    await waybillApi.mechanicCheck(selectedItem.value.id, { passed: checkPassed.value, notes: checkNotes.value })
    ...
    <v-row class="mb-4" align="center">
      <v-col>
        <div class="d-flex align-center gap-3">
          <v-avatar color="warning" size="48" rounded="xl">
            <v-icon color="white" size="24">mdi-wrench</v-icon>
          </v-avatar>
          <div>
            <h2 class="text-h6 font-weight-bold">Mexanik tekshiruvi navbati</h2>
        ...
        <v-data-table :headers="headers" :items="items" :loading="loading" hover rounded="xl">
          ...
          <template #item.actions="{ item }">
            <v-btn color="success" variant="flat" size="small" rounded="lg" class="mr-2" @click="openCheck(item, true)">
              <v-icon start size="14">mdi-check</v-icon> Tasdiqlash
            </v-btn>
    ...
    <v-dialog v-model="checkDialog" max-width="480" persistent>
      <v-card rounded="xl">
        <v-card-title class="pa-5 pb-2">
          ...
          Mexanik xulosasi — {{ selectedItem?.waybill_number }}
        </v-card-title>
        <v-card-text>
          <v-alert :type="checkPassed ? 'success' : 'error'" variant="tonal" rounded="lg" class="mb-3" density="compact">
            {{ selectedItem?.vehicle?.state_number }} — {{ selectedItem?.driver?.full_name }}
          </v-alert>
          <v-textarea v-model="checkNotes" :label="checkPassed ? 'Izoh (ixtiyoriy)' : 'Rad etish sababi (majburiy)'"
